...
import { Plus, X, Check, Loader2, Package, Building2, DoorOpen } from "lucide-react";
...
export default function InventoryBoard() {
  ...
  useEffect(() => {
    ...
    (async () => {
      // A property with no rooms is normal, and so is no property selected —
      // neither should block the whole-property scope, so failures fall back
      // to an empty list rather than surfacing an error.
  ...
  // Rooms already carry their inventory from the list above; only the
  // whole-property scope needs the full property document fetched.
  useEffect(() => {
    let cancelled = false;
  ...
  const save = async () => {
    ...
    try {
      if (scope === WHOLE_PROPERTY) {
        ...
        setRooms((prev) => prev.map((r) => (r._id === scope ? { ...r, inventory } : r)));
      }
  ...
  const scopeLabel = scope === WHOLE_PROPERTY ? "Whole property" : room?.roomName || "Room";
    ...
    <div className="space-y-5">
      ...
      <div className="bg-white border border-gray-100 rounded-2xl p-5 grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div>
          ...
          <select
            ...
            onChange={(e) => { setPropertyId(e.target.value); setScope(WHOLE_PROPERTY); }}
          >
            <option value="">
      ...
      {!propertyId ? (
        <div className="text-center py-16 bg-gray-50 rounded-2xl">
          ...
          <p className="text-gray-500 font-medium">Select a property to view its inventory</p>
        ...
        <>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              ...
              { label: "Total value", value: total ? formatMoney(total) : "—" },
            ].map((s) => (
              <div key={s.label} className="bg-white border border-gray-100 rounded-2xl p-4">
                <p className="text-[10px] font-bold uppercase tracking-widest text-gray-400">{s.label}</p>
                <p className={`font-bold text-[#0F253B] truncate mt-0.5 ${s.small ? "text-sm" : "text-xl"}`}>
                  {s.value}
                </p>
              </div>
            ))}
          ...
          <div className="bg-white border border-gray-100 rounded-2xl p-5 grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              ...
              <input type="date" className={FIELD} value={checkedOn} onChange={(e) => setCheckedOn(e.target.value)} />
            </div>
            <div>
              <label className={LABEL}>Checked by</label>
              <input className={FIELD} value={checkedBy} onChange={(e) => setCheckedBy(e.target.value)} placeholder="e.g., Sarah Khan" />
            </div>
          </div>
          ...
          <div className="bg-white border border-gray-100 rounded-2xl p-5 space-y-4">
            ...
            {items.length === 0 ? (
              <p className="text-center text-sm text-gray-400 font-medium border-2 border-dashed border-gray-100 rounded-2xl py-10">
                No items yet — add what this {scope === WHOLE_PROPERTY ? "property" : "room"} is let with.
              </p>
            ) : (
              <div className="space-y-2">
                <div className="hidden lg:grid lg:grid-cols-[minmax(0,2fr)_minmax(0,1.5fr)_4rem_minmax(0,1fr)_6.5rem_6rem_minmax(0,2fr)_2rem] gap-2 px-1">
                  {["Item", "Location", "Qty", "Condition", "Price", "Value", "Notes"].map((h) => (
                ...
                {items.map((it, i) => (
                  <div
                    key={i}
                    className="grid grid-cols-1 lg:grid-cols-[minmax(0,2fr)_minmax(0,1.5fr)_4rem_minmax(0,1fr)_6.5rem_6rem_minmax(0,2fr)_2rem] gap-2 items-center bg-gray-50 lg:bg-transparent border border-gray-100 lg:border-0 rounded-xl lg:rounded-none p-3 lg:p-0"
                  >
                    <div>
                      <label className={`${LABEL} lg:hidden`}>Item</label>
                      <input className={`${FIELD} bg-white`} value={it.item} onChange={(e) => setItem(i, "item", e.target.value)} placeholder="e.g., Washing machine" />
                    </div>
                    <div>
                      <label className={`${LABEL} lg:hidden`}>Location</label>
                      <input className={`${FIELD} bg-white`} list="inventory-locations" value={it.location} onChange={(e) => setItem(i, "location", e.target.value)} placeholder="Kitchen" />
                    </div>
                    <div>
                      <label className={`${LABEL} lg:hidden`}>Qty</label>
                      <input type="number" min="0" className={`${FIELD} bg-white px-2 text-center`} value={it.quantity} onChange={(e) => setItem(i, "quantity", e.target.value)} />
                    </div>
                    <div>
                      <label className={`${LABEL} lg:hidden`}>Condition</label>
                      <select className={`${FIELD} bg-white px-2`} value={it.condition} onChange={(e) => setItem(i, "condition", e.target.value)}>
                    ...
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <input type="number" min="0" step="0.01" className={`${FIELD} bg-white pl-7 pr-2`} value={it.price} onChange={(e) => setItem(i, "price", e.target.value)} placeholder="0" />
                      </div>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <input className={`${FIELD} bg-white`} value={it.notes} onChange={(e) => setItem(i, "notes", e.target.value)} placeholder="Small scratch on door" />
                    </div>
                    <button
                      type="button"
            ...
            <div className="flex items-center justify-between gap-3 flex-wrap">
              ...
              {items.length > 0 && (
                <div className="flex flex-wrap gap-1.5">
                  {CONDITIONS.map(([v, l]) => {
                    const n = items.filter((it) => it.item.trim() && it.condition === v).length;
                    return n > 0 ? <Badge key={v} tone={CONDITION_TONE[v]}>{n} {l}</Badge> : null;
                  })}
                </div>
              )}
            </div>

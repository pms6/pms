...
const COLUMN_WIDTHS = [38, 14, 14, 12, 22, 40];
 ...
 * the export.
 *
 * `extended` adds the Cleaner and Notes columns the app records but the
 * handwritten sheet had nowhere to put.
 */
export const exportCleaningSheet = async (rows = [], { extended = true } = {}) => {
  const XLSX = await import("xlsx");

  const columns = extended ? [...SHEET_COLUMNS, "Cleaner", "Notes"] : SHEET_COLUMNS;
  ...
  for (const group of groupByMonth(rows)) {
    ...
    for (const row of group.rows) {
      ...
      if (extended) cells.push(row.cleaner || "", row.notes || "");
      aoa.push(cells);
    }
    aoa.push([]);
  }
